...
const SERVICE_CARDS = [
  {
    title: "Home Services",
    description: "Cleaning, plumbing, electrical and more for your home.",
    path: "/home-services",
    icon: Home,
    color: "blue",
  },
  {
    ...
    description: "Book appliance repair and maintenance in a few clicks.",
  ...
  },
  {
    ...
    description: "Schedule servicing and diagnostics for your vehicle.",
...
const COLOR_STYLES = {
  blue: { icon: "text-blue-600", panel: "bg-blue-100 group-hover:bg-blue-200" },
  green: {
    icon: "text-green-600",
    panel: "bg-green-100 group-hover:bg-green-200",
  },
  purple: {
    icon: "text-violet-600",
    panel: "bg-violet-100 group-hover:bg-violet-200",
  },
  red: { icon: "text-red-600", panel: "bg-red-100 group-hover:bg-red-200" },
  amber: {
    icon: "text-amber-600",
    panel: "bg-amber-100 group-hover:bg-amber-200",
  },
} as const;
...
const normalizeBooking = (booking: any): BookingRecord => {
  ...
  const providerDoc = booking?.providerId;
  const serviceTitle =
    serviceDoc?.title ||
    booking?.serviceName ||
    booking?.category ||
    "Service Request";
  const providerName =
    providerDoc?.providerProfile?.companyName ||
    providerDoc?.name ||
    booking?.providerName ||
    "Provider assigned after confirmation";
  ...
  return {
    ...
    price: formatCurrency(serviceDoc?.price ?? booking?.amount ?? 0),
    status: String(booking?.status ?? "pending").toLowerCase(),
    raw: booking,
  };
};

const normalizeTransaction = (tx: any): TransactionRecord => ({
  id: String(tx?.id ?? tx?._id ?? "transaction"),
  amount: Number(tx?.amount ?? 0),
  date: formatDisplayDate(tx?.date),
  status: String(tx?.status ?? "success"),
  description: tx?.description || "Service payment",
  method: tx?.method || "Online",
});

const normalizeFavorite = (favorite: any): FavoriteService => ({
  id: String(favorite?.id ?? favorite?._id ?? favorite?.name ?? "favorite"),
  name: favorite?.name || favorite?.service || "Favorite service",
  category: favorite?.service || favorite?.category || "General",
  frequency: favorite?.frequency || "Saved for quick access",
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          ...
          <Icon
            className={`h-8 w-8 ${COLOR_STYLES[color]?.icon ?? COLOR_STYLES.blue.icon}`}
          />
        </div>
      </CardContent>
    </Card>
  ...
  const QuickActionCard = ({ title, description, path, icon: Icon, color }) => (
    ...
    >
      <CardContent className="p-6 text-center">
        <div
          className={`p-3 rounded-lg inline-block mb-4 transition-colors ${COLOR_STYLES[color]?.panel ?? COLOR_STYLES.blue.panel}`}
        >
          <Icon
            className={`h-8 w-8 ${COLOR_STYLES[color]?.icon ?? COLOR_STYLES.blue.icon}`}
          />
        ...
        >
          ...
          <TabsContent value="overview">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <StatCard
                title="Amount Spent"
                value={formatCurrency(dashboardStats.amountSpent)}
                icon={Receipt}
                color="green"
              />
              <StatCard
